...
def boto_message(user_message):
    boto_says = "I'm at a loss for words. Maybe try my friend Google?"
    if first_message:
        boto_says = process_first_message(user_message)
    else: 
        boto_says = check_for_bad_words(user_message)
    return boto_says
